...
const TenantStaffs = () => {
    ...
    const getAllStaffs = async ({ queryKey }) => {
        const [, selectedOrganizationBranch, page, limit] = queryKey
        try {
            const response = await fetch(`${api}/tenant/all-staffs?branch=${selectedOrganizationBranch}&page=${page}&limit=${limit}`);
            const resBody = await response.json();
            return resBody;
        } catch (error) {
            console.log("Error: ", error);
    ...
    const { data: staffsData, isLoading: isMembersLoading } = useQuery({
        queryKey: ['members', selectedOrganizationBranch, currentPage, limit],
        queryFn: getAllStaffs,
        enabled: !!selectedOrganizationBranch
    });

    const { staffs, totalCount, totalPages } = staffsData || {};
                                            ...
                                            staffs?.map((staff) => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <TableCell>{staff?.tenant?.fullName}</TableCell>
                                                        <TableCell>{staff?.organization?.name}</TableCell>
                                                        <TableCell className="text-start">
                                                            {staff?.organizationBranch?.orgBranchName}
                                                        </TableCell>
                                                        <TableCell className="text-start">
                                                            {staff.contactNo}
                                                        </TableCell>
