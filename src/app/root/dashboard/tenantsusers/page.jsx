...
const TenantsUsers = () => {
    ...
    const getAllUsers = async ({ queryKey }) => {
        ...
        try {
            const response = await fetch(`${api}/tenant/all-system-users?branch=${selectedOrganizationBranch}&page=${page}&limit=${limit}`);
            const resBody = await response.json();
            return resBody;
        } catch (error) {
            console.log("Error: ", error);
    ...
    const { data: usersData, isLoading: isMembersLoading } = useQuery({
        queryKey: ['members', selectedOrganizationBranch, currentPage, limit],
        queryFn: getAllUsers,
        enabled: !!selectedOrganizationBranch
    });

    const { users, totalCount, totalPages } = usersData || {};
    console.log(users);
                                            ...
                                            users?.map((user) => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <TableCell>{user?.tenant?.fullName}</TableCell>
                                                        <TableCell>{user?.organization?.name}</TableCell>
                                                        <TableCell className="text-start">
                                                            {user?.organizationBranch?.orgBranchName}
                                                        </TableCell>
                                                        <TableCell className="text-start">
                                                            {user.phoneNumber}
                                                        </TableCell>
